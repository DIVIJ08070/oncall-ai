 ...
 * physics sandbox: hover and the words collapse into a pile you can shove
 * around, which is the page's message made literal. Purely decorative; the
 * heading and the way back are plain text above it.
 */
export function NotFoundPage() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-bg">
      ...
      {/* ambient accent wash, same language as the console spotlight */}
      <div
      ...
      <header className="relative z-10 flex items-center justify-between px-5 py-5 md:px-8">
        ...
        <Link
          to="/dashboard"
          className="cursor-target rounded-full border border-border-strong px-4 py-2 text-sm font-medium text-ink transition-colors hover:bg-surface-3"
        >
          Dashboard
        </Link>
      </header>
  ...
  return (
    <div className="relative mb-14 flex h-[132px] w-full items-center justify-center">
      <Link
        ...
        className="cursor-target inline-flex items-center gap-2 rounded-full bg-primary px-7 py-3 text-sm font-semibold text-white transition-transform duration-300 ease-out hover:bg-primary-hover"
      >
