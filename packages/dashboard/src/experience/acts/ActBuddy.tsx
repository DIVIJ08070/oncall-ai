...
export function ActBuddy() {
  return (
    <div className="relative h-full w-full">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-[40%] px-6 text-center">
        <p className="ns-seg ns-mono text-xs tracking-[0.35em] text-white/40" style={seg(0.03, 0.28)}>
          NIGHT SHIFT · DAY SHIFT
        </p>
        <h2 className="ns-seg ns-display mt-6 text-5xl text-white sm:text-7xl" style={seg(0.07, 0.31)}>
          Code Review Buddy
        </h2>
        <p className="ns-seg mx-auto mt-5 max-w-md text-white/55" style={seg(0.12, 0.31)}>
          The same AI, reviewing every pull request before a human even opens it.
        </p>
      </div>

      {/* ——— 0.31–0.88: the gigantic editor ——— */}
      <div
        className="ns-seg pointer-events-none absolute left-1/2 top-[38%] w-[min(92vw,58rem)] -translate-x-1/2"
        style={seg(0.31, 0.88, { '--rise': '36px' } as React.CSSProperties)}
      >
        <div className="rounded-sm border border-white/10 bg-[#0A0A0A]/80 shadow-2xl backdrop-blur-sm">
          {/* editor chrome */}
          <div className="ns-mono flex items-center justify-between border-b border-white/10 px-4 py-2.5 text-[10px] uppercase tracking-[0.2em] text-white/40">
            ...
            <span className="relative">
              ...
              <span
                className="buddy-new absolute right-0 top-0 flex items-center gap-2 whitespace-nowrap text-emerald-400"
          ...
          {/* the diff */}
          <div className="ns-mono px-4 py-3 text-left text-[13px] leading-none sm:text-sm">
            <div className="flex h-9 items-center text-white/30">
              <span className="w-10 select-none pr-3" />
              <span className="whitespace-pre">{'@@ -12,3 +12,5 @@ greetUser'}</span>
            </div>

            <div className="flex h-9 items-center">
              <span className="w-10 select-none pr-3 text-right text-white/25">12</span>
              <span className="whitespace-pre text-white/45">{'  async function greetUser(id: string) {'}</span>
            </div>

            <div className="flex h-9 items-center bg-emerald-500/[0.05]">
              <span className="w-10 select-none pr-3 text-right text-white/25">13</span>
            ...
            <div className="relative flex h-9 items-center bg-emerald-500/[0.05]">
              <span className="w-10 select-none pr-3 text-right text-white/25">14</span>
              ...
              <span className="absolute right-3 top-1/2 z-20 hidden w-56 -translate-y-1/2 lg:block">
                <span className="buddy-slide block rounded-sm border border-white/15 bg-[#0D0D0D] p-2.5 text-left text-[10px] leading-relaxed shadow-xl" style={seg(0.55, 0.73)}>
              ...
              <span className="absolute right-3 top-1/2 z-10 -translate-y-1/2">
                <span
                  className="ns-seg inline-flex items-center gap-1.5 rounded-sm border border-emerald-500/40 bg-emerald-500/10 px-2 py-1 text-[10px] text-emerald-400"
            ...
            <div className="relative flex h-9 items-center bg-emerald-500/[0.05]">
              <span className="w-10 select-none pr-3 text-right text-white/25">15</span>
              <span className="relative whitespace-pre text-emerald-300/80">
              ...
              <span className="absolute right-3 top-1/2 z-20 hidden w-60 -translate-y-1/2 lg:block">
                <span className="buddy-slide block rounded-sm border border-white/15 bg-[#0D0D0D] p-3 text-left text-[11px] leading-relaxed shadow-xl" style={seg(0.38, 0.6)}>
            ...
            <div className="flex h-9 items-center">
              <span className="w-10 select-none pr-3 text-right text-white/25">16</span>
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
        <h3 className="ns-seg ns-display text-5xl text-white sm:text-7xl" style={seg(0.87, 1.3)}>
          Ship with confidence.
        </h3>
        <Link
          to="/code-review"
          className="cursor-target ns-mono pointer-events-auto mt-8 inline-block rounded-sm border border-white/20 px-6 py-3 text-xs uppercase tracking-[0.25em] text-white/70 transition-colors hover:text-white"
          style={{
